...
export default function PurchaseOrdersPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      apiListContractsForPO().then(
        (r) => r.success && setContracts(r.data.contracts),
      ),
      apiListCurrencies().then(
        (r) => r.success && setCurrencies(r.data.currencies),
      ),
  ...
  const summaryStats = useMemo(
    () => ({
      ...
      active: orders.filter((o) =>
        ["approved", "sent_to_supplier"].includes(o.status),
      ).length,
  ...
  function updateDraft(updates: Partial<DraftPO>) {
    setModal((prev) => {
      ...
      if (prev.mode === "edit")
        return {
          ...prev,
          draft: { ...(prev.draft || emptyDraft()), ...updates },
        };
      return prev;
    });
  }
  ...
  const draftTotals = useMemo(() => {
    ...
    const subtotal = draft.lineItems.reduce(
      (s, i) => s + computeLineItem(i).lineSubtotal,
      0,
    );
    const tax = draft.lineItems.reduce(
      (s, i) => s + computeLineItem(i).taxAmount,
      0,
    );
  ...
  function handlePrint() {
    ...
    // Defer so the cloned node is laid out before the print dialog opens.
  ...
  async function handleSave() {
    ...
    if (
      ...
      setSaveError(
        "All line items on a trading PO must have a product selected.",
      );
      return;
    }
    ...
    if (draft.lineItems.some((i) => i.orderedQty <= 0)) {
      setSaveError("All quantities must be greater than 0.");
      return;
    }
  ...
  const previewOrder =
    modal && modal.mode !== "new"
      ? ((modal as any).order as PurchaseOrder)
      : null;
      ...
      <section className="space-y-4">
        ...
        <SearchSelect
          ...
          onChange={(val, opt) => {
            ...
            updateDraft({
              contractId: val,
              contractRef: opt.sublabel || "",
              contractTitle: c?.title || opt.label,
            });
          }}
      ...
      <section className="space-y-3">
        ...
        {draft.lineItems.map((item, idx) => {
          ...
          return (
            <div
              key={item._key}
              className="bg-surface rounded-xl border border-border p-3 space-y-3"
            >
              <div className="flex items-center justify-between">
              ...
              <div className="flex items-center justify-between pt-1 border-t border-border text-xs">
                <span className="text-t3">
                  ...
                  <span className="text-t1 font-semibold">
                    {lineSubtotal.toLocaleString()} {draft.currency}
                  </span>
                </span>
                {taxAmount > 0 && (
                  <span className="text-t3">
                ...
                <span className="text-t1 font-bold">
                  Total: {lineTotal.toLocaleString()} {draft.currency}
                </span>
              </div>
            </div>
          );
        ...
        <div className="bg-accent/5 border border-accent/20 rounded-xl p-4 space-y-1.5">
          <div className="flex justify-between text-sm">
            ...
            <span className="font-semibold text-t1">
              {draftTotals.subtotal.toLocaleString()} {draft.currency}
            </span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-t2">Tax</span>
            <span className="font-semibold text-amber-500">
              {draftTotals.tax.toLocaleString()} {draft.currency}
            </span>
          </div>
          <div className="flex justify-between text-base border-t border-accent/20 pt-1.5 mt-1.5">
            ...
            <span className="font-black text-accent">
              {draftTotals.total.toLocaleString()} {draft.currency}
            </span>
          </div>
        </div>
      </section>
    ...
    <div className="space-y-5 pb-10">
      <section className="space-y-3">
        ...
        {[
          ...
          <div key={label} className="flex justify-between text-sm">
            ...
            <span className="font-medium text-t1 text-right max-w-[55%] truncate">
              {val}
            </span>
          </div>
        ))}
      ...
      <section className="space-y-2">
        ...
        {previewOrder.lineItems.map((li, i) => (
          <div
            key={i}
            className="p-3 bg-surface rounded-xl border border-border text-sm space-y-1"
          >
            <div className="font-medium text-t1">{li.description}</div>
            <div className="flex justify-between text-xs text-t3">
              <span>
                {li.orderedQty} {li.unit} × {li.unitPrice.toLocaleString()}
              </span>
              {li.analyticProjectName && (
                <span className="text-accent">{li.analyticProjectName}</span>
              )}
            </div>
            {li.taxRatePercentage > 0 && (
              <div className="text-xs text-amber-500">
                Tax: {li.taxRateName} (+{li.taxAmount.toLocaleString()})
              </div>
            )}
            <div className="text-xs font-bold text-t1 text-right">
              = {li.lineTotal.toLocaleString()} {previewOrder.currency}
            </div>
          </div>
        ))}
      </section>
      ...
      <div className="bg-accent/5 border border-accent/20 rounded-xl p-4 space-y-1.5">
        <div className="flex justify-between text-sm">
          <span className="text-t2">Subtotal</span>
          <span className="font-semibold text-t1">
            {previewOrder.totalValue.toLocaleString()} {previewOrder.currency}
          </span>
        </div>
        {previewOrder.totalTaxAmount > 0 && (
          <div className="flex justify-between text-sm">
            <span className="text-t2">Tax</span>
            <span className="font-semibold text-amber-500">
              {previewOrder.totalTaxAmount.toLocaleString()}{" "}
        ...
        <div className="flex justify-between text-base border-t border-accent/20 pt-1.5 mt-1.5">
          <span className="font-bold text-t1">Grand Total</span>
          <span className="font-black text-accent">
            {previewOrder.totalValueWithTax.toLocaleString()}{" "}
            {previewOrder.currency}
          </span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <button
          type="button"
          onClick={handlePrint}
          className="py-2.5 border border-border text-t1 rounded-xl text-sm font-bold hover:bg-surface transition-all flex items-center justify-center gap-2"
        >
          <Printer size={14} weight="duotone" /> Print
        </button>
        <button
          type="button"
          onClick={() => handleEdit(previewOrder)}
          className="py-2.5 border border-accent text-accent rounded-xl text-sm font-bold hover:bg-accent/5 transition-all"
        >
          Edit
        </button>
      </div>
    ...
    <div className="font-sans text-[#1a1a1a] w-full">
      ...
      <div className="flex justify-between items-start mb-3">
        ...
        <div className="text-right text-[11px] leading-tight text-gray-600">
          <p className="font-bold text-gray-800 uppercase tracking-wider">
            TEKACCESS
          </p>
      ...
      <p className="text-[11px] font-bold text-[#4285f4] mb-8 italic">
        ...
        <div className="text-right text-[12px]">
          ...
          {(orderForPreview as any).vendorReference && (
            <p className="text-gray-400 text-[10px]">
              Ref: {(orderForPreview as any).vendorReference}
            </p>
          )}
          {(orderForPreview as any).contractRef && (
            <p className="text-gray-400 text-[10px]">
              Contract: {(orderForPreview as any).contractRef}
            </p>
          )}
        </div>
      </div>
      ...
      <div className="flex justify-between items-start mb-6 text-[12px] border-y border-gray-100 py-5">
        <div>
          ...
          <p className="text-sm font-bold text-gray-800">
            {(orderForPreview as any).currency}
          </p>
        </div>
        <div>
          <p className="text-[10px] font-black text-gray-400 uppercase mb-1">
      ...
      <div className="mb-6 border border-gray-800 rounded-sm text-[11px]">
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b border-gray-800 bg-gray-50">
              <th className="py-2.5 px-3 text-left font-black uppercase tracking-wider border-r border-gray-800 w-[30%]">
                Product
              </th>
              <th className="py-2.5 px-3 text-left font-black uppercase tracking-wider border-r border-gray-800 w-[20%]">
                Project
              </th>
              <th className="py-2.5 px-3 text-center font-black uppercase tracking-wider border-r border-gray-800">
                Qty / Unit
              </th>
              <th className="py-2.5 px-3 text-right font-black uppercase tracking-wider border-r border-gray-800">
                Unit Price
              </th>
              <th className="py-2.5 px-3 text-right font-black uppercase tracking-wider border-r border-gray-800">
                Tax
              </th>
              <th className="py-2.5 px-3 text-right font-black uppercase tracking-wider">
                Total
              </th>
            </tr>
          </thead>
          <tbody>
            {(orderForPreview as any).lineItems.map((item: any, i: number) => (
              <tr key={i} className="border-b border-gray-200">
                <td className="py-3 px-3 border-r border-gray-200 font-semibold">
                  {item.description}
                </td>
                <td className="py-3 px-3 border-r border-gray-200 text-gray-500 text-[10px]">
                  {item.analyticProjectName || "—"}
                </td>
                <td className="py-3 px-3 text-center border-r border-gray-200">
                  {item.orderedQty} {item.unit}
                </td>
                <td className="py-3 px-3 text-right border-r border-gray-200">
        ...
        <div className="border-t border-gray-300 text-[11px]">
          <div className="flex justify-end px-3 py-1.5 gap-8">
            <span className="text-gray-500 uppercase font-black tracking-wider">
              Subtotal
            </span>
            <span className="font-bold min-w-[100px] text-right">
              {(orderForPreview as any).totalValue.toLocaleString()}{" "}
              {(orderForPreview as any).currency}
            </span>
          </div>
          {(orderForPreview as any).totalTaxAmount > 0 && (
            <div className="flex justify-end px-3 py-1 gap-8 border-t border-gray-100">
              <span className="text-amber-600 uppercase font-black tracking-wider">
                Tax
              </span>
              <span className="font-bold min-w-[100px] text-right text-amber-600">
                +{(orderForPreview as any).totalTaxAmount.toLocaleString()}
              </span>
            </div>
          )}
        </div>
        ...
        <div className="flex justify-end items-stretch border-t border-gray-800 bg-[#851C1C] text-white text-[11px]">
          <div className="py-3 px-6 font-black uppercase border-r border-white/20 tracking-widest">
            Grand Total
          </div>
          <div className="py-3 px-8 font-black text-right min-w-[160px] text-base">
            {(orderForPreview as any).totalValueWithTax.toLocaleString()}{" "}
            {(orderForPreview as any).currency}
          </div>
        </div>
      </div>
  ...
  const monthlyData = useMemo(() => {
    const map: Record<
      string,
      { month: string; orders: number; amount: number }
    > = {};
    orders.forEach((o) => {
  ...
  const categoryData = useMemo(() => {
    ...
    orders.forEach((o) => {
      map[o.procurementType] = (map[o.procurementType] || 0) + 1;
    });
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          >
            ...
            <div>
              <p className="text-xs text-t3 font-medium uppercase tracking-wide">
                {card.label}
              </p>
      ...
      <div className="bg-card rounded-xl border border-border">
          ...
          <div className="flex border border-border rounded-lg overflow-hidden ml-auto">
            {(
              ...
              >
                <v.Icon
                  size={15}
                  weight={viewMode === v.mode ? "fill" : "regular"}
                />
                    ...
                    >
                      ...
                      {colVis.has("status") && (
                        <TableCell>
                          ...
                          >
                            <span
                              className={`w-1.5 h-1.5 rounded-full ${STATUS_DOT[order.status] || STATUS_DOT.draft}`}
                            />
        ...
        {viewMode === "bar" && (
          <div className="p-6">
            <h3 className="text-sm font-semibold text-t2 mb-4">
              Monthly Order Volume & Spend
            </h3>
            ...
            <div className="space-y-3">
              {categoryData.map((item, idx) => (
                <div key={item.category} className="flex items-center gap-3">
                  ...
                  />
                  <span className="text-sm text-t2 flex-1 capitalize">
                  ...
                  <span className="text-sm font-semibold text-t1 w-8 text-right">
                    {item.value}%
                  </span>
                </div>
              ))}
      ...
      <DocumentSidePanel
        ...
        title={
          modal?.mode === "new"
            ? "New Purchase Order"
            : modal?.mode === "edit"
              ? `Edit — ${(modal as any).order?.poRef}`
              : `PO — ${(modal as any)?.order?.poRef}`
        }
        currentIndex={
          modal && modal.mode !== "new"
            ? filteredOrders.findIndex(
                (o) => o._id === (modal as any).order?._id,
              ) + 1
            : undefined
        }
        ...
        onPrev={() => {
          ...
          if (idx > 0)
            setModal({ mode: "view", order: filteredOrders[idx - 1] });
        }}
        onNext={() => {
          if (!modal || modal.mode === "new") return;
          const idx = filteredOrders.findIndex(
      ...
      {showAddClient && (
        <QuickAddClient
          ...
          onCreated={(newClient) => {
            ...
            updateDraft({
              deliverToClientId: newClient._id,
              deliverToClientName: newClient.name,
            });
              ...
              <div className="bg-card border border-border rounded-2xl shadow-2xl p-6 text-center">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  >
                    {isDeleting ? (
                      <>
                        <Spinner size={16} className="animate-spin" /> Deleting…
                      </>
                    ) : (
                      "Delete"
                    )}
