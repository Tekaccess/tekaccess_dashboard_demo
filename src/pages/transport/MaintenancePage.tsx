...
import {
  ...
  Eye, PencilSimple, Spinner, Warning, CheckCircle, X, Wrench, Trash,
} from '@phosphor-icons/react';
import {
  BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Cell, PieChart, Pie,
} from 'recharts';
import {
  apiListMaintenanceRecords, apiCreateMaintenanceRecord, apiUpdateMaintenanceRecord, apiDeleteMaintenanceRecord, MaintenanceRecord,
} from '../../lib/api';
...
export default function MaintenancePage() {
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    await apiDeleteMaintenanceRecord(confirmDelete._id);
    setDeleting(false);
                    ...
                    {colVis.has('actions') && (
                      <td className="px-4 py-3.5">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <button onClick={e => { e.stopPropagation(); setConfirmDelete(record); }} className="p-1.5 text-t3 hover:text-rose-500 hover:bg-rose-500/10 rounded-lg transition-colors"><Trash size={14} weight="duotone" /></button>
                        </div>
                      </td>
                    )}
                  </tr>
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-card border border-border rounded-2xl shadow-2xl p-6 w-full max-w-xs mx-4 text-center">
            ...
            <h2 className="text-base font-bold text-t1 mb-1">Delete Record?</h2>
            <p className="text-xs text-t3 mb-5">
              Record <span className="font-semibold text-t2">{confirmDelete.recordRef}</span> will be permanently removed.
            </p>
