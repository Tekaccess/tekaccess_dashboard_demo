...
import {
  ...
  FileDoc, FileXls, Link, X, Trash,
} from '@phosphor-icons/react';
import { motion, AnimatePresence } from 'motion/react';
import {
  apiGetProcurementContractsSummary, apiListProcurementContracts,
  apiCreateProcurementContract, apiUpdateProcurementContract,
  apiUploadProcurementContractDocument, apiDeleteProcurementContract,
...
const CONTRACT_COLS: ColDef[] = [
  { key: 'contractRef',  label: 'Contract ID', defaultVisible: true },
  { key: 'title',        label: 'Name',         defaultVisible: true },
  { key: 'contractType', label: 'Type',         defaultVisible: true },
...
function emptyDraft(): DraftContract {
  return { contractRef: genRef(), title: '', contractType: 'supplier' };
}
...
export default function ProcurementContractsPage() {
  ...
  const [deleteTarget, setDeleteTarget] = useState<OperationsContract | null>(null);
  const [isDeleting, setIsDeleting] = useState(false);

  const { visible: colVis, toggle: colToggle } = useColumnVisibility('procurement-contracts', CONTRACT_COLS);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const params: Record<string, string> = { page: String(page), limit: '50', isTemplate: 'false' };
    if (search) params.search = search;
    if (statusFilter) params.status = statusFilter;
    if (typeFilter) params.contractType = typeFilter;
    const [cRes, sRes] = await Promise.all([
      apiListProcurementContracts(params),
      apiGetProcurementContractsSummary(),
    ]);
  ...
  function openEdit(c: OperationsContract) {
    setDraft({ contractRef: c.contractRef, title: c.title, contractType: (c.contractType as any) || 'supplier' });
    setSelected(c);
    setError(null);
    setDocFile(null);
    setDocError(null);
  ...
  async function handleDelete() {
    ...
    setIsDeleting(true);
    try {
      await apiDeleteProcurementContract(deleteTarget._id);
  ...
  async function handleSave() {
    ...
    const res = modalMode === 'new'
      ? await apiCreateProcurementContract({ contractRef: draft.contractRef, title: draft.title, contractType: draft.contractType } as any)
      : await apiUpdateProcurementContract(selected!._id, { title: draft.title, contractType: draft.contractType as any });
    ...
    if (docFile) {
      ...
      const docRes = await apiUploadProcurementContractDocument(contractId, docFile);
    ...
    <div className="space-y-5 pb-6">
      ...
      <div>
        ...
        <input className={inp} value={draft.title} onChange={e => setDraft(d => ({ ...d, title: e.target.value }))}
          placeholder="e.g. Supply Agreement — Supplier XYZ Q1 2025" />
      </div>
      ...
      <div>
        ...
        <input ref={docInputRef} type="file"
          ...
          className="hidden" onChange={handleDocPick} />
    ...
    <div className="space-y-5 text-sm">
      <div className="space-y-1">
        ...
        <select className={inp} value={selected.status}
          onChange={async e => { await apiUpdateProcurementContract(selected._id, { status: e.target.value as any }); load(); }}>
      ...
      <div className="pt-2 border-t border-border">
        <button onClick={() => openEdit(selected)}
          className="w-full py-2.5 bg-surface border border-border text-t1 rounded-xl text-sm font-bold hover:bg-card transition-all">
          Edit Contract
        </button>
      </div>
    </div>
      ...
      <div className="bg-card rounded-xl border border-border">
        <div className="flex flex-wrap items-center gap-3 p-4 border-b border-border">
          <div className="relative flex-1 min-w-[180px]">
            ...
            <input type="text" placeholder="Search ref or name…" value={search}
        ...
        <OverlayScrollbarsComponent options={{ scrollbars: { autoHide: 'never' } }} defer>
          <table className="min-w-full divide-y divide-border">
            ...
            <tbody className="bg-card divide-y divide-border">
              {loading ? (
                <tr>
                  <td colSpan={CONTRACT_COLS.length + 1} className="px-4 py-16 text-center">
                    <Spinner size={24} className="animate-spin text-t3 mx-auto" />
                  </td>
                </tr>
              ) : contracts.length === 0 ? (
                <tr>
                  <td colSpan={CONTRACT_COLS.length + 1} className="px-4 py-16 text-center text-t3 text-sm">
                  ...
                  <td className="px-4 py-3.5">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <button onClick={e => { e.stopPropagation(); setDeleteTarget(c); }}
                        className="p-1.5 text-t3 hover:text-rose-500 hover:bg-rose-500/10 rounded-lg transition-colors">
          ...
          <div className="flex items-center justify-between px-4 py-3 border-t border-border text-xs text-t3">
            ...
            <div className="flex gap-2">
              <button disabled={page === 1} onClick={() => setPage(p => p - 1)}
                className="px-3 py-1 text-xs border border-border rounded-lg hover:bg-surface disabled:opacity-40">
                Prev
              </button>
        ...
        <>
          ...
          <motion.div key="del-dlg"
            ...
            <div className="bg-card border border-border rounded-2xl shadow-2xl p-6 text-center">
              ...
              <h2 className="text-base font-bold text-t1 mb-1">Delete contract?</h2>
              <p className="text-xs text-t3 mb-5">
                <span className="font-semibold text-t2">{deleteTarget.contractRef}</span> will be permanently removed.
