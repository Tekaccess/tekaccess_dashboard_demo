...
export default function QuickAddClient({ onClose, onCreated }: QuickAddClientProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.clientCode.trim()) { setError('Client code is required.'); return; }
      ...
      <div className="relative bg-card rounded-2xl shadow-2xl w-full max-w-lg border border-border overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-[10px] text-t3 mb-1">Client Code *</label>
              <input
                value={form.clientCode}
                onChange={e => set('clientCode', e.target.value.toUpperCase())}
                placeholder="e.g. CLT-007"
